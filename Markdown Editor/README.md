# Аналог GitHub‑редактора Markdown

Чтобы сделать **аналог GitHub‑редактора Markdown** (с вкладками «Edit» и «Preview»), понадобится комбинация HTML, CSS, JavaScript и библиотеки для парсинга Markdown.
...
# Markdown на HTML-странице

  

Чтобы отобразить Markdown на HTML-странице в стиле GitHub добавим в `editor.html` кнопку для сохранения HTML файла. Вот обновленный код `editor.html` с новой функцией:

## Обновленный `editor.html` (добавлена кнопка "Сохранить как HTML"):
...
<body>
  <div class="editor-container">
    ...
    <div class="controls">
      ...
      <button onclick="openPreview()">Открыть превью</button>
    </div>
...
<script>
  ...
  // НОВАЯ ФУНКЦИЯ: Сохранение как HTML
  function saveAsHTML() {
    const markdownText = markdownInput.value;
...
<body>
  ...
  // Функция открытия превью в отдельном окне
...
1. **Новая кнопка "💾 Сохранить как HTML"** - зеленого цвета
2. **Функция `saveAsHTML()`** которая:
   - Конвертирует Markdown в HTML
   - Создает полноценный HTML документ с GitHub-стилями
   - Сохраняет файл с именем `preview.html`

## Как использовать:

1. **Напишите Markdown** в редакторе
2. **Нажмите кнопку "Сохранить как HTML"**
3. **Браузер скачает файл `preview.html`**
4. **Откройте скачанный файл** в браузере - он будет выглядеть как превью на GitHub
